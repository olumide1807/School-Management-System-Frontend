...
function StudentListTab() {
  ...
  const referenceTerm = (() => {
    ...
    const upcoming = [...terms].filter((t: any) => t.termStartDate && new Date(t.termStartDate) > now)
      .sort((a: any, b: any) => new Date(a.termStartDate).getTime() - new Date(b.termStartDate).getTime());
    if (upcoming.length > 0) return upcoming[0];
    const completed = [...terms].filter((t: any) => t.termEndDate && endOfDay(new Date(t.termEndDate)) < now)
      .sort((a: any, b: any) => new Date(b.termEndDate).getTime() - new Date(a.termEndDate).getTime());
    return completed[0] || null;
  })();

  const feesInRefTerm = referenceTerm ? allFees.filter((f: any) => f.termId === referenceTerm._id) : [];

  const { data: allPaymentsData } = useQuery({
    queryKey: ['student-fee-payments', referenceTerm?._id, feesInRefTerm.length],
    queryFn: async () => {
      if (feesInRefTerm.length === 0) return [];
      const results = await Promise.all(
        feesInRefTerm.map((fee: any) => SERVER.get(`payment/fee/${fee._id}`).then(r => r?.data?.data || []).catch(() => []))
      );
      return results.flat();
    },
    enabled: !!referenceTerm?._id && feesInRefTerm.length > 0, retry: false,
  });
  ...
  const getStudentFeeStatus = (student: any) => {
    ...
    const totalOwed = fee.fees?.reduce((s: number, f: any) => s + (parseFloat(f.amount) || 0), 0) || 0;
    const paid = refTermPayments.filter((p: any) => p.studentId === student._id && p.feeId === fee._id)
  ...
  const headcells = [
    ...
    { key: "status", name: "Status" },
    {
      ...
      name: [
        {
          name: "View Profile",
          handleClick: (row: any) => navigate(`/student-management/student-profile/${row._raw._id}`),
        },
        {
          name: "Record Fee Payment",
          handleClick: (row: any) => navigate(`/school-management/fee-management`),
        },
        {
          name: "Print ID Card",
          handleClick: (row: any) => {
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-6">
        ...
        <Button color="tertiary" variant="contained" startIcon={<PersonAdd />}
        ...
        <div className="flex flex-wrap items-center gap-3 mb-6">
          <div className="flex items-center border border-gray-300 rounded-lg px-3 py-2 flex-1 min-w-[200px] max-w-[400px]">
            ...
            <input type="text" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}
              placeholder="Search by name or ID..." className="flex-1 outline-none text-sm" />
          </div>
          <FormControl size="small" sx={{ minWidth: 150 }}>
            <InputLabel>Class</InputLabel>
            <Select value={filterClass} label="Class" onChange={(e) => setFilterClass(e.target.value)} sx={{ borderRadius: "10px" }}>
              <MenuItem value="All">All Classes</MenuItem>
              {classArms.map((arm: any) => <MenuItem key={arm._id} value={arm._id}>{getArmLabel(arm._id)}</MenuItem>)}
            </Select>
          </FormControl>
          <FormControl size="small" sx={{ minWidth: 120 }}>
            <InputLabel>Gender</InputLabel>
            <Select value={filterGender} label="Gender" onChange={(e) => setFilterGender(e.target.value)} sx={{ borderRadius: "10px" }}>
      ...
      {students.length === 0 ? (
        <div className="flex items-center justify-center h-[280px] border border-gray-100 rounded-lg">
          <div className="flex flex-col items-center gap-3">
            ...
            <Button color="tertiary" variant="contained" onClick={() => navigate("/school-management/admission")}
              sx={{ color: "white", borderRadius: "10px", textTransform: "capitalize" }}>Admit Student</Button>
          </div>
        </div>
        ...
        <div className="flex items-center justify-center h-[280px] border border-gray-100 rounded-lg">
          <div className="flex flex-col items-center gap-3">
            ...
            <Button variant="outlined" color="tertiary"
              onClick={() => { setSearchTerm(""); setFilterClass("All"); setFilterGender("All"); setFilterStatus("All"); }}
              sx={{ borderRadius: "10px", textTransform: "capitalize" }}>Clear filters</Button>
          </div>
        </div>
      ) : (
        <TableComponent headcells={headcells} tableData={tableData} message="No students found" />
      )}
    </>
  );
}
...
function ParentListTab() {
  ...
  const filteredParents = parents.filter((p: any) => {
    ...
    return name.includes(q) || p.email?.toLowerCase().includes(q) || p.phoneNumber?.includes(q);
  });

  const getLinkedStudents = (parentId: string) => {
    return students.filter((s: any) => s.guardians?.some((g: any) => g.parentId === parentId));
  };
  ...
  const tableData = filteredParents.map((parent: any, i: number) => {
    ...
    return {
      ...
      guardian: (
        <div className="flex gap-x-3 items-center">
          <Avatar sx={{ width: 36, height: 36 }}>{(parent.firstName?.[0] || "P")}</Avatar>
          <div className="flex flex-col">
            <p className="text-sm font-medium">{parent.title || ""} {parent.firstName} {parent.surName}</p>
            <p className="text-xs text-gray-500">{parent.email || "-"}</p>
          </div>
        </div>
      ),
      phone: parent.phoneNumber || "-",
      wards: linked.length > 0
  ...
  const headcells = [
    ...
    { key: "wardCount", name: "Linked" },
    {
      ...
      name: [
        {
          name: "View Profile",
          handleClick: (row: any) => navigate(`/student-management/parent-profile/${row._raw._id}`),
        },
      ],
    },
  ];
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-6">
        <p className="text-gray-500 text-sm">All registered parents and guardians</p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        ...
        <div className="border border-gray-200 rounded-xl p-4 bg-green-50">
          ...
          <p className="text-2xl font-bold text-green-700">
            {parents.filter((p: any) => getLinkedStudents(p._id).length > 0).length}
          </p>
        </div>
      </div>
      ...
      {parents.length > 0 && (
        <div className="flex items-center border border-gray-300 rounded-lg px-3 py-2 max-w-[400px] mb-6">
          <Search fontSize="small" className="text-gray-400 mr-2" />
          <input type="text" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}
            placeholder="Search by name, email, or phone..." className="flex-1 outline-none text-sm" />
        </div>
      )}

      {parents.length === 0 ? (
        <div className="flex items-center justify-center h-[280px] border border-gray-100 rounded-lg">
          <p className="text-gray-500">No parents registered yet. Parents are added during student admission.</p>
        </div>
      ) : (
        <TableComponent headcells={headcells} tableData={tableData} message="No parents match your search" />
      )}
    </>
  );
}