...
import EditStudentData from "./EditStudentTabs/EditStudentData";
import ReviewEditedStudent from "./EditStudentTabs/ReviewEditedStudent";

export default function EditStudentProfile({ openModal, closeModal }) {
  const [presentStep, setPresentStep] = useState(1);
  const [allData, setAllData] = useState({});

  const stepComponents = {
    1: <EditStudentData />,
    2: <ReviewEditedStudent data={allData} />,
  };
        ...
        <div className="flex items-center">
          {steps.map((step) => (
            <button
              ...
              className={`text-center items-center flex flex-col gap-y-2.5 text-lg ${
                step.id === presentStep
                  ? "custom-rule2 relative text-tertiary"
                  : "text-buttonInactive"
              } py-2.5 px-16`}
            >
              <p>Step {step.id}</p>
