  ...
  return (
    <div className="max-w-[800px]">
      ...
      <p className="text-sm text-gray-500 mb-6">
        Manage your profile, school, and system preferences
      </p>
...
function MyProfileTab() {
  ...
  const { data: profileData, isPending } = useQuery({
    ...
    queryFn: async () => {
      const res = await SERVER.get("superadmin/profile");
      return res?.data;
    },
    retry: false,
  });
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await SERVER.put("superadmin/update", {
        firstName,
        lastName,
        emailAddress: email,
        phoneNumber: phone,
  ...
  const handlePhotoUpload = async (file: File) => {
    ...
    try {
      ...
      fd.append("file", file);
      await SERVER.post("superadmin/upload", fd, {
        ...
        {[
          ...
          <div key={label} className="flex flex-col">
            ...
            {editing ? (
              ...
              <p className="text-sm font-medium text-black py-2.5">
                {val || "—"}
              </p>
            )}
          </div>
...
function SchoolDetailsTab() {
  ...
  useEffect(() => {
    if (settings) setPrincipalId(settings.principalId || "");
  }, [settings]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await SERVER.put("settings", { principalId: principalId || null });
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="bg-white border border-gray-200 rounded-xl p-6">
        ...
        <h3 className="font-semibold text-black mt-6 mb-1">Principal</h3>
        <p className="text-xs text-gray-500 mb-4">
          Whose name and comment appear on report cards. Access is controlled
          separately by the staff member's admin status.
        </p>
        {editing ? (
          <select
            value={principalId}
            onChange={(e) => setPrincipalId(e.target.value)}
            className="border border-gray-300 rounded-lg p-2.5 text-sm bg-white max-w-sm"
          >
            <option value="">Not set</option>
            {allStaff.map((s: any) => (
              <option key={s._id} value={s._id}>
                {`${s.title ? s.title + " " : ""}${s.firstName || ""} ${s.surname || ""}`.trim()}
              </option>
            ))}
          </select>
        ) : (
          <p className="text-sm font-medium text-black py-2.5">
            {allStaff.find((s: any) => s._id === principalId)
              ? `${allStaff.find((s: any) => s._id === principalId).firstName} ${allStaff.find((s: any) => s._id === principalId).surname}`
              : "Not set"}
          </p>
        )}
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
          <div className="flex flex-col">
            <label className="text-xs text-gray-500 mb-1">Session</label>
            <p className="text-sm font-medium py-2.5">
              {activeSession?.sessionName || "No active session"}
            </p>
          </div>
          <div className="flex flex-col">
            <label className="text-xs text-gray-500 mb-1">Current Term</label>
            <p className="text-sm font-medium py-2.5">
              {activeTerm?.termName || "No active term"}
            </p>
          </div>
          <div className="flex flex-col">
            <label className="text-xs text-gray-500 mb-1">Term Period</label>
            <p className="text-sm font-medium py-2.5">
              {activeTerm?.termStartDate && activeTerm?.termEndDate
                ? `${new Date(activeTerm.termStartDate).toLocaleDateString()} — ${new Date(activeTerm.termEndDate).toLocaleDateString()}`
                : "—"}
            </p>
          </div>
        </div>
...
function AcademicSettingsTab() {
  ...
  useEffect(() => {
    if (settingsData?.data?.gradingScale)
      setGrades(settingsData.data.gradingScale);
    if (settingsData?.data?.attendanceSettings) {
      setSchoolStartTime(
        settingsData.data.attendanceSettings.schoolStartTime || "08:00",
      );
      setGracePeriodMinutes(
        settingsData.data.attendanceSettings.gracePeriodMinutes ?? 15,
      );
    }
  }, [settingsData]);
  ...
  useEffect(() => {
    ...
    if (s) {
      ...
      setEndTime(s.endTime || "14:00");
      setPeriodDuration(s.periodDuration || 40);
      setBreaks(s.breaks || []);
    }
  }, [periodData]);
  ...
  const addGrade = () =>
    setGrades([...grades, { grade: "", minScore: 0, maxScore: 0, remark: "" }]);
          ...
          <div className="mt-4 bg-gray-50 rounded-lg px-4 py-2 text-xs text-gray-600">
            ...
            <strong>
              {(() => {
                ...
                cutoff.setHours(h, m + gracePeriodMinutes, 0);
            ...
            will be marked{" "}
            <span className="text-green-700 font-medium">Present</span>. After
            that → <span className="text-yellow-700 font-medium">Late</span>.
          </div>
        )}
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1">
              Define what score ranges map to each grade
            </p>
          </div>
          <div className="flex gap-2">
            <Button
        ...
        <div className="border border-gray-200 rounded-lg overflow-hidden">
          ...
          {grades.map((g: any, i: number) => (
            <div
              key={i}
              className="px-4 py-2 grid grid-cols-[60px_1fr_1fr_1fr_40px] gap-3 border-t border-gray-100 items-center"
            >
              <input
                value={g.grade || ""}
                onChange={(e) => updateGrade(i, "grade", e.target.value)}
                className="border border-gray-300 rounded p-1.5 text-sm text-center w-full"
                maxLength={2}
              />
              <input
                type="number"
                value={g.minScore ?? ""}
                onChange={(e) => updateGrade(i, "minScore", e.target.value)}
                className="border border-gray-300 rounded p-1.5 text-sm w-full"
              />
              <input
                type="number"
                value={g.maxScore ?? ""}
                onChange={(e) => updateGrade(i, "maxScore", e.target.value)}
                className="border border-gray-300 rounded p-1.5 text-sm w-full"
              />
              <input
                value={g.remark || ""}
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1">
              Configure school hours, period duration, and breaks
            </p>
          </div>
          <Button
            variant="contained"
        ...
        <div className="flex items-center justify-between mb-3">
          ...
          <Button
            ...
            onClick={() =>
              setBreaks([
                ...breaks,
                { name: "Break", startTime: "10:00", endTime: "10:30" },
              ])
            }
            sx={{ textTransform: "capitalize", color: "#0E7094" }}
          >
        ...
        {breaks.length === 0 ? (
          <p className="text-sm text-gray-400 py-2">No breaks configured</p>
        ) : (
            ...
            {breaks.map((b: any, i: number) => (
              ...
              >
                <input
                  ...
                  placeholder="Name"
                  className="border border-gray-300 rounded p-1.5 text-sm flex-1"
                />
                <input
                  type="time"
                  value={b.startTime}
...
function FeesAndIdsTab() {
  ...
  const [platformFeeDesc, setPlatformFeeDesc] = useState(
    "Platform Fee (Basitech)",
  );
  ...
  useEffect(() => {
    ...
    if (s) {
      ...
      setPlatformFeeDesc(
        s.feeDefaults?.platformFeeDescription || "Platform Fee (Basitech)",
      );
    ...
    try {
      await SERVER.put("settings", {
        feeDefaults: {
          ...
          platformFeeDescription: platformFeeDesc,
        },
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1">
              Default currency and platform fee for all fee breakdowns
            </p>
          </div>
          <Button
            variant="contained"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
          ...
          <div className="flex flex-col">
            ...
            <input
              value={platformFeeDesc}
              onChange={(e) => setPlatformFeeDesc(e.target.value)}
              className="border border-gray-300 rounded-lg p-2.5 text-sm"
            />
          </div>
        </div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-6">
        ...
        <p className="text-xs text-gray-500 mb-4">
          Set the prefix for auto-generated student and staff IDs
        </p>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          <div className="flex flex-col">
            <label className="text-xs text-gray-500 mb-1">
...
function NotificationsTab() {
  ...
  useEffect(() => {
    if (settingsData?.data?.notifications)
      setNotifs({ ...notifs, ...settingsData.data.notifications });
  }, [settingsData]);
  ...
  const NOTIFICATION_OPTIONS = [
    {
      key: "emailOnStudentRegistration",
      label: "Student Registration",
      desc: "Send email when a new student is registered",
    },
    {
      key: "emailOnFeePayment",
      label: "Fee Payment",
      desc: "Send email when a fee payment is recorded",
    },
    {
      key: "emailOnStaffAdded",
      label: "Staff Added",
      desc: "Send email when a new staff member is created",
    },
    {
      key: "emailOnAttendanceMarked",
      label: "Attendance Marked",
      desc: "Send email when daily attendance is saved",
    },
    {
      key: "emailOnPasswordReset",
      label: "Password Reset",
      desc: "Send email when a password is reset",
    },
  ];
      ...
      <div className="flex flex-col gap-1">
        {NOTIFICATION_OPTIONS.map((opt) => (
          ...
          >
            ...
            <Switch
              checked={(notifs as any)[opt.key]}
              onChange={(e) =>
                setNotifs({ ...notifs, [opt.key]: e.target.checked })
              }
              color="success"
            />
          </div>
...
function ChangePasswordTab() {
  ...
  const handleChangePassword = async () => {
    ...
    if (newPassword.length < 6) {
      toast.error("Min 6 characters", toastOptions);
      return;
    }
    if (currentPassword === newPassword) {
      toast.error("New password must be different", toastOptions);
      return;
    }
    setSaving(true);
    try {
      await SERVER.put("superadmin/updatepassword", {
        currentPassword,
        newPassword,
      });
    ...
    <div className="flex flex-col">
      ...
      <div className="relative">
        <input
          ...
          placeholder="••••••••"
          className="border border-gray-300 rounded-lg p-2.5 text-sm w-full pr-10"
        />
        <IconButton
          size="small"
          onClick={toggle}
    ...
    <div className="bg-white border border-gray-200 rounded-xl p-6 max-w-[500px]">
      ...
      <div className="flex flex-col gap-5">
        ...
        {newPassword && newPassword.length < 6 && (
          <p className="text-xs text-orange-600">Min 6 characters</p>
        )}
        <Button
          variant="contained"
          color="tertiary"
